#!/usr/bin/env python3
"""
Experiment: Train model on single course vs all courses
Hypothesis: Course-specific models might perform better
"""
...
print("=" * 70)
print("EXPERIMENT: Single Course vs All Courses")
print("=" * 70)
...
def train_ensemble_cv(X_train_df, y, n_folds=5, verbose=True):
...
# ============================================================
# EXPERIMENT 1: Global model (all courses)
# ============================================================
print("\n[2/4] Training global model (all courses)...")
...
# ============================================================
# EXPERIMENT 2: Per-course models
# ============================================================
print("\n[3/4] Training per-course models...")
...
for course in courses:
    ...
    X_course = train[course_mask].drop(['id', target], axis=1)
    y_course = train[course_mask][target].values

    print(f"    Samples: {len(X_course):,} ({len(X_course)/len(train)*100:.1f}%)")

    # Remove course column since it's constant
    X_course_no_course = X_course.drop('course', axis=1)

    results = train_ensemble_cv(X_course_no_course, y_course, verbose=False)
...
print("\n  COMPARISON TABLE:")
print("-" * 70)
print(f"  {'Course':<12} | {'Samples':>10} | {'XGB':>8} | {'CAT':>8} | {'HGB':>8} | {'Ensemble':>10}")
print("-" * 70)
...
for course in courses:
    n_samples = len(train[train['course'] == course])
    r = course_results[course]
    diff = r['ensemble'] - global_results['ensemble']
    print(f"  {course:<12} | {n_samples:>10,} | {r['xgb']:>8.5f} | "
          f"{r['cat']:>8.5f} | {r['hgb']:>8.5f} | {r['ensemble']:>10.5f} ({diff:+.5f})")

print("-" * 70)

# Calculate weighted average of per-course models
print("\n  WEIGHTED AVERAGE OF PER-COURSE MODELS:")
total_samples = len(train)
weighted_ensemble = 0
for course in courses:
    n_samples = len(train[train['course'] == course])
    weight = n_samples / total_samples
    weighted_ensemble += weight * course_results[course]['ensemble']
...
print("\n  BEST/WORST PERFORMING COURSES (vs Global):")
diffs = {c: course_results[c]['ensemble'] - global_results['ensemble'] for c in courses}
sorted_diffs = sorted(diffs.items(), key=lambda x: x[1])

print("    Better than global:")
for c, d in sorted_diffs:
    if d < 0:
        print(f"      {c}: {d:+.5f}")

print("    Worse than global:")
for c, d in sorted_diffs:
    if d >= 0:
        print(f"      {c}: {d:+.5f}")
...
if weighted_ensemble < global_results['ensemble']:
    print(f"\n  Per-course models are BETTER by {global_results['ensemble'] - weighted_ensemble:.5f}")
    print("  Consider training separate models per course.")
else:
    print(f"\n  Global model is BETTER by {weighted_ensemble - global_results['ensemble']:.5f}")
    print("  The global model generalizes well across courses.")
